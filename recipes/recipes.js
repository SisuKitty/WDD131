...
function getRandomListEntry(list) {
    ...
    const randomNumber = randomNum(listLength);
...
function renderRecipes(recipeList) {
    ...
    recipeString += recipeTemplate(recipeList[i]);
    }
	// Set the HTML strings as the innerHTML of our output element.
    console.log(recipeString);
    output.innerHTML = recipeString;
}
...
function filterList(input) {
	const filtered = recipes.filter((recipe) => recipe.tags.find((item) => item.toLowerCase().includes(input)));
	// sort by name
...
function searchHandler(event) {
    ...
    const filtered = filterList(lowerInput);
...
//     const filteredRecipes = searchList(query);
//     renderRecipes(filteredRecipes);
// });

// function searchList(list, obj) {
//     function stringContains(item) {
//         return (
//             item.toLowerCase().includes(obj.toLowerCase()) ||
//             item.toLowerCase().includes(obj.toLowerCase()) ||
//             item.tags.find((tag) => tag.toLowerCase().includes(obj.toLowerCase()))
//     );
//     }
//     const filtered = list.filter(stringContains);
